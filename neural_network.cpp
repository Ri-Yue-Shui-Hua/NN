...
void NeuralNetwork::train(std::vector<std::vector<float>>& train_xs,
    ...
    size_t trainingDataSize = train_xs.size();
    size_t testDataSize = test_xs.size();
    ...
    for (int e = 0; e < epoch_; ++e) {
        ...
        // abort();
        cout << "第" << e << "轮已完成,耗时: " << fixed << duration<double>(system_clock::now() - t).count() << "秒,准确率：";
        cout.flush();
        cout << fixed << this->getAccuracy(testXvecs, testYvecs) * 100 << R"(%,loss: )" << fixed << trainingLoss / trainingDataSize << endl;
    }
};
float NeuralNetwork::getAccuracy(std::vector<Eigen::VectorXf>& test_xvs,
    ...
    size_t testDataSize = test_xvs.size();
