...
#include <iostream>
#include <random>
[[maybe_unused]] static float relu(float f) { return std::max(f, 0.0f); }
[[maybe_unused]] static float derive_relu(float f) { return (f > 0 ? f : 0); }

[[maybe_unused]] static float tanh_(float f) { return std::tanh(f); }
[[maybe_unused]] static float derive_tanh(float f) {
